...
const AllConstructors = () => {
  ...
  useEffect(() => {
    const fetchConstructors = async () => {
      try {
        ...
        const limitedData = data.slice(0, 4).map((item, index) => ({
          ...item,
          imageUrl:
            item.imageUrl ||
            `https://source.unsplash.com/400x300/?building,construction,${index}`,
        }));
  ...
  const fetchConstructorById = async (id) => {
    try {
      const token = sessionStorage.getItem("token");
      const api = axios.create({
        baseURL: "http://localhost:3000/api",
      ...
      const response = await api.get(`/constructors/${id}`);
      const constructorData = response.data;
      console.log("Fetched constructor data:", constructorData);
    } catch (error) {
  ...
  return (
    <section className="py-4 bg-gradient-to-r from-purple-200 via-blue-100 to-blue-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-12">
                ...
                <div className="mt-4 flex justify-between items-center text-gray-600 text-sm">
                  <div className="flex items-center">
                    ...
                    >
                      ...
                      <span className="ml-1">
                        {likes[constructor._id || constructor.id] || 0}
                      </span>
                    </button>
        ...
        <div className="mt-12 text-center">
          <button
            onClick={() => navigate("/all-constructors")}
            className="py-3 px-8 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-md shadow-md hover:bg-blue-600 focus:outline-none"
          >
            See More
          </button>
        </div>
